...
class SubmitToRegistrarService
  ...
  def call
    ...
    generate_augmented_pdf

    PsRegistrarService.call(submission:) if Settings.dor_submit_ps_xml
  end
  ...
  def generate_augmented_pdf
    augmented_pdf_path = SignaturePageService.call(submission: submission)
    submission.augmented_dissertation_file.attach(io: File.open(augmented_pdf_path),
