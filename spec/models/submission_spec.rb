...
RSpec.describe Submission do
  ...
  describe '#alert_if_accessioning_started' do
    before do
      allow(Honeybadger).to receive(:notify)
    end

    context 'when accessioning has started' do
      subject(:submission) { create(:submission, :accessioning_started) }

      it 'notifies Honeybadger when the submission is updated' do
        submission.update!(title: 'Updated title')
        ...
        expect(Honeybadger).to have_received(:notify).with(
          ...
          context: hash_including(
            dissertation_id: submission.dissertation_id,
            sunetid: submission.sunetid,
            druid: submission.druid,
            changes: hash_including('title' => [anything, 'Updated title'])
          )
        )
      end
    end

    context 'when accessioning has not started' do
      subject(:submission) { create(:submission) }

      it 'does not notify Honeybadger when the submission is updated' do
        submission.update!(title: 'Updated title')
