...
RSpec.describe SubmissionPoster do
  ...
  context 'when submitting via HTTP Basic' do
    before do
      allow(Settings.peoplesoft).to receive(:oauth_enabled).and_return(false)

      stub_request(:post, Settings.ps_reg_endpoint)
        .with(
          headers: {
    ...
    describe '.call' do
      let(:instance) { instance_double(LegacySubmissionPoster, call: nil) }

      before do
        allow(LegacySubmissionPoster).to receive(:new).and_return(instance)
      end
    ...
    describe '#call' do
      let(:fake_response) do
        instance_double(Faraday::Response, status:, body: 'OK')
      end
      let(:service) { LegacySubmissionPoster.new(submission:) }
      let(:status) { 200 }
