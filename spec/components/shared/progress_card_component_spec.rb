...
RSpec.describe Shared::ProgressCardComponent, type: :component do
  ...
  context 'when rendering student steps' do
  ...
  context 'when not rendering student steps' do
    it 'renders the list of steps' do
      render_inline(described_class.new(submission:, with_student_steps: false))
      expect(page).to have_css('aside h2', text: 'Progress')
