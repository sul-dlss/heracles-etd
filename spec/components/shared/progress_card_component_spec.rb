...
RSpec.describe Shared::ProgressCardComponent, type: :component do
  ...
  before do
    ...
    allow(submission_presenter).to receive(:step_done?).with(8).and_return(true)
    allow(submission_presenter).to receive(:step8_done?).and_return(true)
  end
  ...
  it 'renders the list of steps' do
    ...
    expect(page).to have_css('.character-circle-success', count: 6)
    expect(page).to have_css('.character-circle-blank', count: 2)
    expect(page).to have_css('#step-8 .text-muted', text: 'January  1, 2023 12:00pm')
  end
end
